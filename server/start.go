...
func startInProcess(ctx *server.Context, clientCtx client.Context, opts StartOptions, app types.Application) (err error) {
	...
	if config.API.Enable || config.JSONRPC.Enable {
		...
		if config.GRPC.Enable {
			...
			logger.Info("anh Tai gRPC address", "port", port)
			if err != nil {
			...
			ctx.Logger.Info("anh Tai gRPC client assigned to client context", "address", grpcAddress)
	...
	if config.GRPC.Enable {
		ctx.Logger.Info("anh Tai starting gRPC server", "server", config.GRPC.Address)
